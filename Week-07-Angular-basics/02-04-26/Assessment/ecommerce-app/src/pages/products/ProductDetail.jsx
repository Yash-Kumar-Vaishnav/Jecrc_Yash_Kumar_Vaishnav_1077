  ...
  return (
    <div className="product-detail fade-up">
      <div className="detail-breadcrumb">
        <Link to="/products">{t.nav ? t.nav.products : 'Products'}</Link>
        <span>/</span>
        <span>{p.name}</span>
      </div>
      ...
      <div className="detail-hero">
        <div className="detail-thumb">
          ...
          {p.badge && (
            <span className={`badge ${p.badge === 'Sale' ? 'badge-rust' : p.badge === 'Popular' ? 'badge-green' : 'badge-gold'} detail-badge`}>
              {p.badge}
            </span>
          )}
        </div>
        <div className="detail-info">
          ...
          <div className="detail-rating">
            ...
            <span className="detail-rating-count">({p.reviews} {t.products.verifiedReviews})</span>
          </div>
          <p className="detail-desc">{p.desc}</p>
          <p className="detail-price">{p.price}</p>
          <div className="detail-actions">
            <button className="btn btn-primary" style={{ flex: 1 }}>{t.products.addToCart}</button>
            <button className="btn btn-outline">{t.products.wishlist}</button>
          </div>
          <div className="detail-meta">
            <span>🚚 {t.products.freeShipping}</span>
            <span>↩️ {t.products.returns}</span>
      ...
      <div className="detail-tabs-wrap">
        <nav className="detail-tabs">
          <NavLink to="reviews" className="detail-tab">{t.products.reviews}</NavLink>
          <NavLink to="specs"   className="detail-tab">{t.products.specs}</NavLink>
        </nav>
